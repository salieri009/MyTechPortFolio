...
This project is a fully implemented portfolio website in a local development environment, designed to be deployed to a cloud environment (Azure or AWS) optimized architecture considering scalability, stability, and maintainability. The goal is to maximize development productivity by separating frontend and backend development and automating the deployment process through CI/CD pipelines.
...
1. **Portfolio Management**: Project, Academic info, Tech Stack CRUD
...
6. **Internationalization**: Multi-language support via React i18next (Korean, English, Japanese)
7. **Responsive Design**: Perfect user experience across all devices
8. **API Versioning**: `/api/v1` based RESTful API
...
1. **Build**: `./gradlew build` → Creates JAR file
2. **Test**: Automatic unit and integration test execution
3. **Deployment**: Deploy JAR file to Azure App Service
...
1. **Build**: `./gradlew build` → Creates JAR file
2. **Test**: Automatic unit and integration test execution
3. **Deployment**: Upload JAR file to Elastic Beanstalk
...
- **Connection**: Use MongoDB connection string
...
- **CORS**: Environment-specific allowed origins configuration
...
- **Frontend**: Automatic scaling via CDN
- **Backend**: 
...
- **Elastic Beanstalk**: EC2 instance cost
